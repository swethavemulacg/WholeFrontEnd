...
@Component({
  selector: 'app-show-booking-history',
  templateUrl: './show-booking-history.component.html',
  styleUrls: ['./show-booking-history.component.css']
})
export class ShowBookingHistoryComponent implements OnInit {
  ...
  *ngOnInit() is a place to put the code that we need to execute at very 
  first as soon as the class is instantiated
  */
  ngOnInit(): void {
    /* it will call the method from service to get the booking history if present */
    this.bookingService.bookingHistory().subscribe(data=>{
